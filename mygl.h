...
float Variacao(int x, int y){

	float soma = (x ^ 2) + (y ^2);
	
	float taxaVariacao = sqrt(soma);
...
tRGBA Interpolacao(tRGBA color1, tRGBA color2, float taxaVariacao){ // Pegamos a variação das cores, onde são dadas por: (cor1 - cor2) / variação

	float variacaoRed = (color1.red - color2.red) / taxaVariacao;
	float variacaoGreen = (color1.green - color2.green) / taxaVariacao;
	float variacaoBlue = (color1.blue - color2.blue) / taxaVariacao;
	float variacaoAlpha = (color1.alpha - color2.alpha) / taxaVariacao;
	tRGBA color;
